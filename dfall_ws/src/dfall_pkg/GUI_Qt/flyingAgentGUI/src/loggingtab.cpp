    ...
    ui(new Ui::LoggingTab)
{
    ...
    ROS_INFO_STREAM("[LOGGING TAB GUI] ros::this_node::getNamespace() =  " << this_namespace);
    ...
    {
        ROS_ERROR("[LOGGING TAB GUI] Node NOT FUNCTIONING :-)");
        ros::spin();
    }
    ...
    {
        setpointChangedSubscriber = nodeHandle_for_this_gui.subscribe("DeepcControllerService/SetpointChanged", 1, &LoggingTab::setpointChangedCallback, this);
    }

    // CREATE THE CUSTOM BUTTON PRESSED PUBLISHER
    customButtonPublisher = nodeHandle_for_this_gui.advertise<dfall_pkg::CustomButtonWithHeader>("Logging/CustomButtonPressed", 1);
...
#ifdef CATKIN_MAKE
void LoggingTab::publish_custom_button_command(int button_index , QLineEdit * lineEdit_pointer)
{
    ...
    ROS_INFO_STREAM("[LOGGING TAB GUI] button " << button_index << " clicked.");
}
#endif


void LoggingTab::on_custom_button_1_clicked()
{
#ifdef CATKIN_MAKE
...
void LoggingTab::setMeasuredPose(float x , float y , float z , float roll , float pitch , float yaw , bool occluded)
{
    if (!occluded)
    {
...
void LoggingTab::poseDataUnavailableSlot()
{
...
#ifdef CATKIN_MAKE
void LoggingTab::setpointChangedCallback(const dfall_pkg::SetpointWithHeader& newSetpoint)
{
    // INITIALISE A STRING VARIABLE FOR ADDING THE "+"
    QString qstr = "";
...
void LoggingTab::publishSetpoint(float x, float y, float z, float yaw_degrees)
{
#ifdef CATKIN_MAKE
    // Initialise the message as a local variable
    ...
    // Inform the user about the change
    ROS_INFO_STREAM("[LOGGING GUI] Published request for setpoint change to: [" << x << ", "<< y << ", "<< z << ", "<< yaw_degrees << "]");
#else
    // TO ASSIST WITH DEBUGGING WHEN COMPILED AND RUN IN "QtCreator"
    QTextStream(stdout) << "[LOGGING GUI] would publish request for: [" << x << ", "<< y << ", "<< z << ", "<< yaw_degrees << "]";
#endif
}
...
void LoggingTab::setAgentIDsToCoordinate(QVector<int> agentIDs , bool shouldCoordinateAll)
{
    ...
    if (agentIDs.length() == 1)
    {
        ...
        {
            // Inform the user
            ROS_INFO("[LOGGING GUI] Failed to get setpoint from controller using the \"GetCurrentSetpoint\" service");
        }

        // SUBSCRIBERS
        // > For receiving message that the setpoint was changed
        setpointChangedSubscriber = agent_base_nodeHandle.subscribe("DeepcControllerService/SetpointChanged", 1, &LoggingTab::setpointChangedCallback, this);
    }
    else
    {
        // Unsubscribe
...
void LoggingTab::fillSetpointMessageHeader( dfall_pkg::SetpointWithHeader & msg )
{
    switch (m_type)
    {
        case TYPE_AGENT:
        ...
        default:
        {
            ...
            ROS_ERROR("[LOGGING TAB GUI] The 'm_type' variable was not recognised.");
            break;
        }
    }
}
...
#ifdef CATKIN_MAKE
// Fill the header for a message
void LoggingTab::fillCustomButtonMessageHeader( dfall_pkg::CustomButtonWithHeader & msg )
{
    switch (m_type)
    {
        case TYPE_AGENT:
...
#ifdef CATKIN_MAKE
bool LoggingTab::getTypeAndIDParameters()
{
    ...
    if(!nodeHandle.getParam("type", type_string))
    {
        ...
        ROS_ERROR("[LOGGING TAB GUI] Failed to get type");
    }
    ...
    else
    {
        ...
        ROS_ERROR("[LOGGING TAB GUI] The 'type' parameter retrieved was not recognised.");
    }
    ...
    switch (m_type)
    {
        case TYPE_AGENT:
        {
            ...
            if(!nodeHandle.getParam("agentID", m_ID))
            {
                // Throw an error if the agent ID parameter could not be obtained
                return_was_successful = false;
                ROS_ERROR("[LOGGING TAB GUI] Failed to get agentID");
            }
            else
            {
                // Inform the user about the type and ID
                ROS_INFO_STREAM("[LOGGING TAB GUI] Is of type AGENT with ID = " << m_ID);
            }
            break;
        }
        ...
        case TYPE_COORDINATOR:
        {
            ...
            if(!nodeHandle.getParam("coordID", m_ID))
            {
                ...
                return_was_successful = false;
                ROS_ERROR("[LOGGING TAB GUI] Failed to get coordID");
            }
            else
            {
                // Inform the user about the type and ID
                ROS_INFO_STREAM("[LOGGING TAB GUI] Is of type COORDINATOR with ID = " << m_ID);
            }
            break;
        }
